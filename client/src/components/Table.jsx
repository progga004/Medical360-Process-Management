  ...
  const { id_to_department, id_to_equipment, deleteEquipment, deletePatient, deleteRoom, deleteUser, getPatient, getEquipment, getRoom } = useGlobalContext();
    if (context === "patient") {
      newCards = cards.map((patient) => {
        return { // Ensure to include a return statement
          ...patient,
          ...
          onClick: () => navigate(`/patient-info/${patient._id}`) // Add navigation function for patient rows
        };
      });
            ...
            <table className="min-w-full">
                ...
                <tbody>
                    {newCards.map((card, index) => (
                        <tr key={index} onClick={() => card.onClick && card.onClick()} style={{ backgroundColor: index % 2 === 0 ? "#EDF2FB" : "#ABC4FF" }}>
                            {fields.map((field, i) => (
